...
- [~] **Next fault at 0x01819DA0** — inside a system library (below our binary's 0x7fa71000 load region). `bad insn = 0xA93F002C = lha r9, 44(r31)` with r31=0 → NULL+44 deref. Prime suspect: our `rumpuser_malloc → osal_malloc → IExec->AllocVecTags` chain returning NULL for some odd size/alignment and rump code dereferencing without checking. osal_malloc now traces every call — next session should read `T:trumpinit.log` to see the last (size, align) before the fault.
