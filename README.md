...
- [~] **`test_rump_init` RUNS on-guest** — verified: launches, prints "calling rump_init()...", control transfers into rump kernel code, GrimReaper fires somewhere inside `rump_init`.
- [x] **Crash pinned via SetTaskTrap catcher** — `tests/phase2/test_rump_init.c` installs `tc_TrapCode` handlers for every exception class, saves ExceptionContext on fault, redirects `ip` to a `crash_dump()` running on a preallocated recovery stack. First fault caught: **PRIV_VIOL at link-time `0x01003a28`, instruction `mfsprg r9, 0`**. Enclosing function `mi_cpu_init` (`sys/kern/subr_cpu.c`) — NetBSD's per-CPU cookie via PowerPC SPRG0, privileged on this Book-E, unavailable in user mode.
- [x] **mfsprg / mtsprg emulator VERIFIED on-guest** — `tests/phase2/test_trap_emul.c` (a standalone 69 KB test with no rump) executes `mfsprg r9, 0` deliberately, trap catches, emulator patches ip+=4 and sets r9=&g_fake_cpu, user code sees the expected pointer. `pass=YES, emulated count=1`.
- [x] **Rump gets past mi_cpu_init** — with mfsprg emulator active, `priv-emulated count=1 last_spr=272` is logged before the next fault.
- [x] **uvmexp pointer stub fix** — our `rump_link_stubs.c` had `uint32_t uvmexp_pagemask = 4095` but NetBSD declares it as `const int *const`. `PAGE_MASK = *uvmexp_pagemask` was dereferencing the value 4095 as a pointer and DSI-faulting at DEAR=0xFFF. Fixed by giving them their real pointer type backed by static ints. Confirmed on-guest: rump gets past kmem_intr_alloc's pagesize math.
- [~] **Next fault at 0x01822DB4** — inside a system library. `bad insn = 0x98090000 = stb r0, 0(r9)` with r9=2 → writes byte 0 to address 2. Deterministic (same PC across runs). Full early rump init sequence now traced via osal_trace to `T:trumpinit.log`: `rumpuser_init`, `curlwpop CREATE/SET`, 3× `getparam`, then 6× (malloc+mtx_init) + 3× (malloc+cv_init) + `clock_gettime`, then fault. All osal calls returned valid pointers. Crash is inside a loaded library, not our code.
- [x] **xdftool direct-deploy workflow** — bridge-drop bug on 823 KB pushes reliably wedges the bridge. Sidestep: `pkill qemu` → `xdftool amigaos4-dev.hdf write local target` → restart QEMU → `curl launch`. Fully documented in `[[host_side_hdf_editing]]` memory. Enables tight kernel-init test iteration.
