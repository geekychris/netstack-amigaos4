...
- [~] **mfsprg / mtsprg emulator** — trap handler now decodes the offending X-form insn; on `mfspr rT, 272..279` returns our shadow `g_sprg[N]` (SPRG0 lazily = `&g_fake_cpu`, 4 KB writable); `mtspr` writes to the shadow. Any other privileged insn still routes to crash_dump so we learn what to emulate next. **Untested on-guest yet — bridge dropped repeatedly mid-transfer during commit iteration.**
